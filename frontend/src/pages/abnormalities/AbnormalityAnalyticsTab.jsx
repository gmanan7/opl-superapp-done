...
import { Layers, Tag, UserCheck, User, Users, Filter, TrendingUp, Download, ClipboardList } from 'lucide-react';
...
import { useAbnormalityAnalytics, useAbnormalityAnalyticsTrend } from '../../hooks/useAbnormalities';
import { EmptyState } from '@/components/patterns';

// Same visual language as OPL Analytics (src/pages/opl/OPLList.jsx) — kept as a local copy
// rather than a shared import since each module's analytics tab evolves independently.
const CHART_GRID = '#F0EFEE';
const CHART_AXIS = '#A8A29E';
const CHART_PALETTE = ['#B45309', '#0E7490', '#7C3AED', '#B91C1C', '#15803D', '#C2410C', '#1D4ED8', '#A16207'];

const ABN_TYPE_LABEL = {
  minor_flaw: 'Minor Flaw',
  unfulfilled_basic_condition: 'Unfulfilled Basic Condition',
  source_of_contamination: 'Source of Contamination',
  inaccessible_place: 'Inaccessible Place',
  source_of_quality_defect: 'Source of Quality Defect',
  unnecessary_item: 'Unnecessary Item',
...
// toISOString() converts to UTC, which rolls the date back a day in timezones ahead of UTC
// (e.g. IST, +5:30) — format using local date parts instead.
...
export function AbnormalityAnalyticsTab() {
  const now = new Date();
  ...
  // null = global (every group in the plant); otherwise scopes every section below.
  ...
  const ANALYTICS_TABS = [
    ...
    { key: 'category', label: 'Type & Tag Color', short: 'Type/Tag', Icon: Tag },
    { key: 'status', label: 'Status & Responsibility', short: 'Status', Icon: ClipboardList },
  ...
  const analyticsParams = filterType === 'jh_group' && breakdownGroupId
    ...
    : filterType === 'dmt' && breakdownDmtId
      ...
      : appliedRange;
  const { data, isLoading, isError } = useAbnormalityAnalytics(analyticsParams);

  // Always-unscoped fetch, purely to populate the JH Group / DMT pickers regardless of what's
  // currently selected — dedupes via React Query's cache when filterType is 'all'.
  const { data: unscopedData } = useAbnormalityAnalytics(appliedRange);
  ...
  const { data: compareDataA } = useAbnormalityAnalytics(compareAId ? { ...appliedRange, [compareKey]: compareAId } : appliedRange);
  const { data: compareDataB } = useAbnormalityAnalytics(compareBId ? { ...appliedRange, [compareKey]: compareBId } : appliedRange);

  // Trend tab state — one JH group at a time, month-over-month.
  const [trendJhGroupId, setTrendJhGroupId] = useState(null);
  const [trendMonths, setTrendMonths] = useState(6);
  const { data: trendData, isLoading: isTrendLoading } = useAbnormalityAnalyticsTrend(
    { jh_group_id: trendJhGroupId, months: trendMonths },
    { enabled: Boolean(trendJhGroupId) }
  );

  // Per-User Stage Breakdown: filter to one status to see just who's there, by name.
  const [perUserStageFilter, setPerUserStageFilter] = useState('all');
  // Member Submissions: bucketed 0 / exactly N / more than N, with an editable threshold.
  ...
  const handleResetToMonth = () => {
    setFromDate(monthStartStr); setToDate(todayStr);
    setAppliedRange({ from: monthStartStr, to: todayStr });
  };
  const handleResetToPastMonth = () => {
    setFromDate(pastMonthStartStr); setToDate(pastMonthEndStr);
  ...
  const byResponsibility = data?.byResponsibility || [];
  const tagColor = data?.tagColor || { red: 0, white: 0 };
  const reviewerWorkload = data?.reviewerWorkload || [];
  const submitterStageBreakdown = data?.submitterStageBreakdown || [];
  const totalAbnormalities = byDmt.reduce((sum, d) => sum + d.abnormality_count, 0);
  ...
  const participationByGroupId = useMemo(() => {
    ...
    memberSubmissions.forEach((m) => {
      ...
      if (m.abnormality_count > 0) map.set(m.jh_group_id, map.get(m.jh_group_id) + 1);
    });
    return map;
  }, [memberSubmissions]);
  ...
  const perUserStageOptions = useMemo(() => (
    [...new Set(submitterStageBreakdown.map((r) => r.stage_label))]
  ), [submitterStageBreakdown]);

  const memberBuckets = useMemo(() => {
    const zero = memberSubmissions.filter((m) => m.abnormality_count === 0);
    const exact = memberSubmissions.filter((m) => m.abnormality_count === memberThreshold);
    const more = memberSubmissions.filter((m) => m.abnormality_count > memberThreshold);
    return { zero, exact, more };
  }, [memberSubmissions, memberThreshold]);

  const handleExportAll = () => {
    const hasAnyData = byDmt.length || byJhGroup.length || memberSubmissions.length
      || byType.length || reviewerWorkload.length || submitterStageBreakdown.length;
    if (!hasAnyData) return toast.error('No data to export for this date range');
    ...
    addSheet('By DMT', byDmt.map((r) => ({ DMT: r.dmt_name, Count: r.abnormality_count })));
    addSheet('By JH Group', byJhGroup.map((r) => ({
      'JH Group': r.jh_group_name, DMT: r.dmt_name, Count: r.abnormality_count,
      Members: r.member_count, Index: r.abnormality_index,
    })));
    addSheet('By Type', byType.map((r) => ({ Type: ABN_TYPE_LABEL[r.type] || r.type, Count: r.abnormality_count })));
    addSheet('By Status', byStatus.map((r) => ({ Status: r.status_label, Count: r.abnormality_count })));
    addSheet('By Responsibility', byResponsibility.map((r) => ({ Responsibility: r.responsibility_name, Count: r.abnormality_count })));
    addSheet('Reviewer Workload', reviewerWorkload.map((r) => ({ Reviewer: r.worker_name, Stage: r.stage_name, Count: r.abnormality_count })));
    addSheet('Per-User Breakdown', submitterStageBreakdown.map((r) => ({ User: r.worker_name, Stage: r.stage_label, Count: r.abnormality_count })));
    addSheet('Member Submissions', memberSubmissions.map((r) => ({ Member: r.worker_name, 'JH Group': r.jh_group_name, Count: r.abnormality_count })));
    XLSX.writeFile(workbook, `Abnormality_Analytics_${appliedRange.from}_to_${appliedRange.to}.xlsx`);
    ...
    <div className="space-y-4 py-4">
      ...
      <div className="rounded-xl border border-line bg-surface-raised p-3 sm:p-4 shadow-xs space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-line-subtle">
          ...
          <select
            value={filterType === 'jh_group' ? (breakdownGroupId || '') : ''}
            onChange={(e) => e.target.value && selectJhGroupFilter(e.target.value)}
            className="h-8 rounded-full border border-line bg-surface-sunken px-2 text-xs text-ink-strong"
          >
          ...
          <select
            value={filterType === 'dmt' ? (breakdownDmtId || '') : ''}
            onChange={(e) => e.target.value && selectDmtFilter(e.target.value)}
            className="h-8 rounded-full border border-line bg-surface-sunken px-2 text-xs text-ink-strong"
          >
      ...
      {/* Section tabs — mobile: a 3-per-row grid with short labels so every section is
          ...
          filter rows in AbnormalityList). Desktop: the single pill row. */}
      ...
      <div className="hidden sm:flex flex-wrap items-center gap-1 bg-surface-sunken p-1 rounded-lg border border-line">
        {ANALYTICS_TABS.map(({ key, label, Icon }) => (
          <button
            ...
            type="button"
            onClick={() => setAnalyticsTab(key)}
            className={`flex shrink-0 items-center gap-1.5 whitespace-nowrap px-2.5 py-1.5 rounded-md text-xs font-semibold transition-all ${
              analyticsTab === key ? 'bg-surface-raised text-blue-700 shadow-xs' : 'text-ink-muted hover:text-ink-strong'
            }`}
          >
            <Icon size={13} /> {label}
      ...
      {/* The date range defaults to month-to-date, so early in a month — or for a plant that
          simply hasn't reported yet — every chart legitimately reads zero. Say so, and offer
          the one tap that usually fixes it, instead of leaving a page of empty charts. */}
      {!isLoading && totalAbnormalities === 0 && (
        <div className="rounded-xl border border-amber-200 bg-amber-500/5 p-3 text-sm text-amber-900 flex flex-col sm:flex-row sm:items-center gap-2">
          <span className="flex-1">
            No abnormalities were reported between <strong>{appliedRange.from}</strong> and{' '}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            <div className="rounded-xl border border-line bg-surface-raised p-4 text-center">
              <p className="text-2xl font-bold text-ink-strong">{totalAbnormalities}</p>
              <p className="text-xs text-ink-muted uppercase tracking-wider">Total Reported</p>
            </div>
            <div className="rounded-xl border border-line bg-surface-raised p-4 text-center">
              <p className="text-2xl font-bold text-ink-strong">{totalMembers}</p>
              <p className="text-xs text-ink-muted uppercase tracking-wider">Members</p>
            </div>
            <div className="rounded-xl border border-line bg-surface-raised p-4 text-center">
              <p className="text-2xl font-bold text-ink-strong">{totalMembers > 0 ? (totalAbnormalities / totalMembers).toFixed(2) : '—'}</p>
              <p className="text-xs text-ink-muted uppercase tracking-wider">Overall Index</p>
            </div>
          </div>

          <div className="rounded-xl border border-line bg-surface-raised p-3 sm:p-4 shadow-xs">
            <p className="text-sm font-semibold text-ink-strong mb-2">By DMT</p>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={byDmt}>
                <CartesianGrid strokeDasharray="3 3" stroke={CHART_GRID} vertical={false} />
                <XAxis dataKey="dmt_name" tick={{ fontSize: 11, fill: CHART_AXIS }} tickLine={false} axisLine={{ stroke: CHART_GRID }} />
                <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: CHART_AXIS }} tickLine={false} axisLine={false} width={28} />
                <Tooltip />
                <Bar dataKey="abnormality_count" radius={[4, 4, 0, 0]} onClick={(d) => selectDmtFilter(d.dmt_id)} cursor="pointer">
          ...
          <div className="rounded-xl border border-line bg-surface-raised p-3 sm:p-4 shadow-xs">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={byJhGroup}>
                ...
                <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: CHART_AXIS }} tickLine={false} axisLine={false} width={28} />
                <Tooltip />
                <Bar dataKey="abnormality_count" radius={[4, 4, 0, 0]} onClick={(d) => selectJhGroupFilter(d.jh_group_id)} cursor="pointer">
          ...
          {/* Six columns can't fit a phone — below sm each row becomes a labelled card
              instead of a side-scrolling table; the real table returns from sm up. */}
            ...
            {byJhGroup.map((g) => (
              <button key={g.jh_group_id} type="button" onClick={() => selectJhGroupFilter(g.jh_group_id)}
                ...
                <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
                  {[
                    ['Reported', g.abnormality_count],
                    ['Members', g.member_count],
                    ['Index', g.abnormality_index ?? '—'],
          ...
          <div className="hidden sm:block rounded-xl border border-line bg-surface-raised overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="bg-surface-sunken text-ink-muted uppercase tracking-wider text-2xs">
                <tr><th className="text-left p-2.5">JH Group</th><th className="text-left p-2.5">DMT</th><th className="text-right p-2.5">Count</th><th className="text-right p-2.5">Members</th><th className="text-right p-2.5">Index</th><th className="text-right p-2.5">Participation</th></tr>
              </thead>
              <tbody>
                {byJhGroup.map((g) => (
                  <tr key={g.jh_group_id} className="border-t border-line-subtle hover:bg-surface-hover cursor-pointer" onClick={() => selectJhGroupFilter(g.jh_group_id)}>
                    ...
                    <td className="p-2.5 text-right">{g.abnormality_count}</td>
                    <td className="p-2.5 text-right">{g.member_count}</td>
                    <td className="p-2.5 text-right">{g.abnormality_index ?? '—'}</td>
      ...
      {!isLoading && analyticsTab === 'category' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="rounded-xl border border-line bg-surface-raised p-3 sm:p-4 shadow-xs">
            <p className="text-sm font-semibold text-ink-strong mb-2">By Type</p>
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie data={byType} dataKey="abnormality_count" nameKey="type" outerRadius={100} label={renderPctLabel} labelLine={false}>
                  {byType.map((row, idx) => <Cell key={row.type} fill={CHART_PALETTE[idx % CHART_PALETTE.length]} />)}
                </Pie>
                <Tooltip formatter={(v, n) => [v, ABN_TYPE_LABEL[n] || n]} />
                <Legend formatter={(v) => ABN_TYPE_LABEL[v] || v} wrapperStyle={{ fontSize: 11 }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
          <div className="rounded-xl border border-line bg-surface-raised p-3 sm:p-4 shadow-xs">
            <p className="text-sm font-semibold text-ink-strong mb-2">Red vs White Tag</p>
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie
                  data={[{ name: 'Red Tag', value: tagColor.red }, { name: 'White Tag', value: tagColor.white }]}
                  dataKey="value" nameKey="name" outerRadius={100} label={renderPctLabel} labelLine={false}
                >
                  <Cell fill="#B91C1C" />
                  <Cell fill="#A8A29E" />
                </Pie>
      ...
      {!isLoading && analyticsTab === 'status' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="rounded-xl border border-line bg-surface-raised p-3 sm:p-4 shadow-xs">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie data={byStatus} dataKey="abnormality_count" nameKey="status_label" outerRadius={100} label={renderPctLabel} labelLine={false}>
          ...
          <div className="rounded-xl border border-line bg-surface-raised p-3 sm:p-4 shadow-xs">
            <p className="text-sm font-semibold text-ink-strong mb-2">By Responsibility</p>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={byResponsibility} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke={CHART_GRID} horizontal={false} />
                <XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: CHART_AXIS }} tickLine={false} axisLine={false} />
                <YAxis type="category" dataKey="responsibility_name" tick={{ fontSize: 11, fill: CHART_AXIS }} tickLine={false} axisLine={false} width={90} />
                <Tooltip />
                <Bar dataKey="abnormality_count" radius={[0, 4, 4, 0]}>
                  {byResponsibility.map((row, idx) => <Cell key={row.responsibility_name} fill={CHART_PALETTE[idx % CHART_PALETTE.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
            ...
            <tbody>
              {reviewerWorkload.length === 0 ? (
                ...
                <tr key={`${r.emp_id}|${r.stage_name}`} className="border-t border-line-subtle">
                  ...
                  <td className="p-2.5 text-right font-semibold">{r.abnormality_count}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="rounded-xl border border-line bg-surface-raised p-3">
              ...
              <div className="max-h-48 overflow-y-auto space-y-1">
                {memberBuckets.zero.map((m) => <p key={m.emp_id} className="text-2xs text-ink-muted">{m.worker_name}</p>)}
              </div>
            </div>
            <div className="rounded-xl border border-line bg-surface-raised p-3">
              <p className="text-xs font-semibold text-ink-strong mb-1">Exactly {memberThreshold} ({memberBuckets.exact.length})</p>
              <div className="max-h-48 overflow-y-auto space-y-1">
                {memberBuckets.exact.map((m) => <p key={m.emp_id} className="text-2xs text-ink-muted">{m.worker_name}</p>)}
              </div>
            </div>
            <div className="rounded-xl border border-line bg-surface-raised p-3">
              <p className="text-xs font-semibold text-ink-strong mb-1">More than {memberThreshold} ({memberBuckets.more.length})</p>
              <div className="max-h-48 overflow-y-auto space-y-1">
                {memberBuckets.more.map((m) => <p key={m.emp_id} className="text-2xs text-ink-muted">{m.worker_name} ({m.abnormality_count})</p>)}
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[{ id: compareAId, set: setCompareAId, data: compareDataA }, { id: compareBId, set: setCompareBId, data: compareDataB }].map((slot, idx) => {
              ...
              const total = slot.data
                ? (compareType === 'jh_group' ? slot.data.byJhGroup?.[0]?.abnormality_count : slot.data.byDmt?.[0]?.abnormality_count) || 0
                : 0;
              return (
                <div key={idx} className="rounded-xl border border-line bg-surface-raised p-4">
                  ...
                  <p className="text-2xs text-ink-muted uppercase tracking-wider text-center">Reported</p>
                </div>
              );
            })}
          </div>
      ...
      {analyticsTab === 'trend' && (
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row gap-2">
            <select value={trendJhGroupId || ''} onChange={(e) => setTrendJhGroupId(e.target.value || null)} className="w-full sm:w-auto h-9 rounded-lg border border-line bg-surface-raised px-2.5 text-xs text-ink-strong">
              <option value="">Select JH Group...</option>
              {allJhGroupOptions.map((g) => <option key={g.jh_group_id} value={g.jh_group_id}>{g.jh_group_name}</option>)}
            </select>
            <select value={trendMonths} onChange={(e) => setTrendMonths(Number(e.target.value))} className="w-full sm:w-auto h-9 rounded-lg border border-line bg-surface-raised px-2.5 text-xs text-ink-strong">
            ...
            <div className="rounded-xl border border-line bg-surface-raised p-3 sm:p-4 shadow-xs">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={trendData?.months || []}>
                  ...
                  <Tooltip />
                  <Legend wrapperStyle={{ fontSize: 11 }} />
                  <Line type="monotone" dataKey="abnormality_count" name="Reported" stroke="#B45309" strokeWidth={2} />
                  <Line type="monotone" dataKey="participants" name="Participants" stroke="#0E7490" strokeWidth={2} />
                  <Line type="monotone" dataKey="abnormality_index" name="Index" stroke="#7C3AED" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
